...
# 日志输出路径
LOGS_DIR=/wdata/weblog/mycs-service-member-server
# 主启动类
MAIN_CLASS=cn.mycs.service.member.server.MemberServerApp
...
# 从配置文件取得应用名、端口号，端口名
SERVER_NAME=`sed '/name/!d;s/.*://' $CONFIG_FILE | tr -d '\r'`
SERVER_NAME=${SERVER_NAME%#*}
SERVER_PORT=`sed '/port/!d;s/.*://' $CONFIG_FILE | tr -d '\r'`
...
JAVA_DEBUG_OPTS=""
if [ "$1" = "debug" ]; then
    JAVA_DEBUG_OPTS=" -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,address=18520,server=y,suspend=n "
fi
JAVA_JMX_OPTS=""
if [ "$1" = "jmx" ]; then
